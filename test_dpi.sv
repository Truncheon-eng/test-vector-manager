...
module test_dpi;
    ...
    class my_test extends uvm_test;
        ...
        task run_phase(uvm_phase phase);
            ...
            /*
             * В SystemVerilog int имеет размер 32 бита,
             * то есть 4 байта.
             */
            shared_memory_size =
            ...
            /*
             * Массив, который будет записан в shared memory.
             */
            ...
            /*
             * Изначально очищаем принимающий массив.
             */
            ...
            `uvm_info(
                "SV",
                $sformatf(
                    "Truncating shared memory to %0d bytes",
                    shared_memory_size
                ),
                UVM_LOW
            )

            result = shared_memory_truncate(
                fd,
                shared_memory_size
            );
            ...
            if (result != 0) begin
                `uvm_fatal(
                    "SV",
                    "shared_memory_truncate() failed"
                )
            end


            /*
             * Сначала считываем 8 int из разделяемой памяти.
             */
            `uvm_info(
                "SV",
                "Reading array from shared memory",
                UVM_LOW
            )
            ...
            /*
             * Записываем 8 int в разделяемую память.
             */
            result = write_array(
                fd,
                write_data_array,
